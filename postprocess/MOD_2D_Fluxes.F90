...
real(r4), parameter :: spval = -1.e36_r4    ! a special value for missing and filling use
real(r8), parameter :: spval_r8 = -1.e36_r8 ! a special value for missing and filling use
...
real(r8), allocatable :: f_tlsun  (:,:)  ! sunlit leaf temperature [K]
real(r8), allocatable :: f_tlsha  (:,:)  ! shaded leaf temperature [K]
...
real(r8), allocatable :: f_z0ma   (:,:)  ! effective roughness [m]
...
SUBROUTINE allocate_2D_Fluxes(lon_points,lat_points,nl_soil,maxsnl,nl_lake)
! --------------------------------------------------------------------
...
allocate ( f_tlsun  (lon_points,lat_points) )  ! sunlit leaf temperature [K]
allocate ( f_tlsha  (lon_points,lat_points) )  ! shaded leaf temperature [K]
...
allocate ( f_z0ma   (lon_points,lat_points) )  ! effective roughness [m]
...
deallocate ( f_tlsun  )  ! sunlit leaf temperature [K]
deallocate ( f_tlsha  )  ! shaded leaf temperature [K]
...
deallocate ( f_z0ma   )  ! effective roughness [m]
